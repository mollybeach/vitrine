...
//uint256 []
let gameCycleWinTokenIDs = [];
let fullGameWinTokenIDs = [];

//address []
let gameCycleWinAddresses = [];
let fullGameWinAddresses = [];

function enterLotto(_totalTicketsSold, _totalCreatedTributes) {
    gameLevel = (_totalTicketsSold / _totalCreatedTributes) / (interval/100);
    return gameLevel;
}
...
function sweepStakes(_winSlots, _prize) {
    for (let i = 0; i < _winSlots; i++) {
        // loverAddress.transferFrom(address(this), draw(_tributes), _prize);
        winningTokenID = draw(
            (gameLevel - 1) * interval,
            gameLevel * interval
        );
        gameCycleWinTokenIDs.push(winningTokenID);
        fullGameWinTokenIDs.push(winningTokenID);
        for (let key in addressToTokenId) {
            if (addressToTokenId[key].includes(winningTokenID)) {
                gameCycleWinAddresses.push(key);
                fullGameWinAddresses.push(key);
            }
        }
    }
    totalTokensPicked = _winSlots;
    console.log(
        "GAME CYCLE # " +
            gameLevel +
            "\n  \n PRIZE : " +
            _prize +
            " $LOVER TOKENS DEPOSITED" +
            "\n \n \n" +
            gameCycleWinTokenIDs.length +
            " TOKENID WINNERS : " +
            "\n     " +
            gameCycleWinTokenIDs +
            "\n \n" +
            gameCycleWinAddresses.length +
            " ADDRESSES WINNERS : " +
            gameCycleWinAddresses +
            "\n \n \n"
    );

    totalAddressesRewarded += gameCycleWinAddresses.length;
    totalWinTokensRewarded += gameCycleWinTokenIDs;
    gameCycleWinAddresses = [];
    gameCycleWinTokenIDs = [];
}

function launch(_totalTicketsSold, _totalCreatedTributes) {
    gameLevel = enterLotto(_totalTicketsSold, _totalCreatedTributes);
    if (gameLevel === 1) {
        sweepStakes(40, 100);
    } else if (gameLevel === 2) {
        sweepStakes(30, 500);
        ...
        console.log("buyers : " + _totalTicketsSold);
        console.log("gameLevel :" + gameLevel);
        console.log("not enough Tribute NFT buyers to launch the next raffle");
    }
}
launch(25, 100);
launch(50, 100);
