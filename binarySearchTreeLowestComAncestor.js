...
function BinarySearchTreeLCA(strArr) { 
    ...
    let family = JSON.parse(strArr[0]);
    let rootNode = family[0] ;
    if(rootNode === babyOne | rootNode === babyTwo) {
      lca = rootNode
      return lca
    }; 
    let youngerSibling = Math.min(babyOne, babyTwo);
    let oldestSibling = Math.max(babyOne, babyTwo);
    let lca = youngerSibling;
    for (let i = family.indexOf(youngerSibling); i >= 0; i--) {
      console.log(typeof family[i] )
      if(youngerSibling <= family[i] && family[i] <= oldestSibling){
         lca = family[i]
      }
    }
     return lca
